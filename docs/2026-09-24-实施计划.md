...
用 MCP Inspector 连一次，确认工具列表与资源能在 UI 里看到（不填真实 token 也能看列表）：

```bash
QQDL_TOKEN=placeholder npx @modelcontextprotocol/inspector node dist/src/index.js manage
```

Windows PowerShell 写法：`$env:QQDL_TOKEN='placeholder'; npx @modelcontextprotocol/inspector node dist/src/index.js manage`。
