...
- 2026-09-24 追加：14 个写操作工具加确认层 `src/confirm.ts`：客户端支持 elicitation 就弹操作预览由用户点确认（`inputRequired` 多轮，2025/2026 两版协议都通），不支持就退到一次性 `confirm_token`；`--yes` 关闭；单测 6 个、端到端 6 个，共 73 用例。用户拍板：A 为主 B 兜底，全部写操作硬确认，接受与客户端自带权限弹窗叠加。
- 2026-09-24 至 2026-09-25 追加（各有对应提交）：子账号 id 类参数统一 string；派生工具机制（overrides 顶层 `derived` + `ToolDef.pick`）与 `user_balance`；token 改为网站 API Key，文档站下线登录类接口后同步为 81 页；每个请求附带 `is_mcp_send=1` 与 UA；弹窗被拒绝或取消时退到确认码而不是直接取消；`order_renew` 先查子账号当前自动续费设置再确认（`src/renew.ts`）；`setup --npx` 与 MIT 许可证；确认码静默期 `QQDL_CONFIRM_QUIET_MS`；分发改为 GitHub（`npx -y github:empty-byte/quanqiudaili-mcp`），npm 注册受阻暂缓。
